...
from dataset import SegMultiHeadList, ConcatSegList
import data_transforms as transforms
from min_norm_solvers import MinNormSolver
from model import CerberusSegmentationModelMultiHead
...
def mIoU(output, target):
    ...
    hist += np.bincount(num_classes * target[k].astype(
        int) + pred[k], minlength=num_classes**2).reshape(num_classes, num_classes)
...
class Cerberus():
    def __init__(self, yaml_path):
        ...
        self.task_list = [['Wood', 'Painted', 'Paper', 'Glass', 'Brick', 'Metal', 'Flat', 'Plastic', 'Textured', 'Glossy', 'Shiny'],
                          ['L', 'M', 'R', 'S', 'W'],
                          ['Segmentation']]
        ...
        if self.mode == 'train':
            ...
            if config['random_rotate'] > 0:
                train_tf.append(transforms.RandomRotateMultiHead(
                    config['random_rotate']))
            if config['random_scale'] > 0:
                train_tf.append(transforms.RandomScaleMultiHead(
                    config['random_scale']))

            train_tf.extend([transforms.RandomCropMultiHead(config['random_crop']),
            ...
            dataset_at_train = SegMultiHeadList(
                config['data_dir'], 'train_attribute', train_tf, out_name=True)
            dataset_af_train = SegMultiHeadList(
                config['data_dir'], 'train_affordance', train_tf, out_name=True)
            dataset_seg_train = SegMultiHeadList(
                config['data_dir'], 'train', train_tf, out_name=True)

            self.train_loader = DataLoader(
                ConcatSegList(dataset_at_train, dataset_af_train,
                              dataset_seg_train),
                batch_size=config['batch_size'], shuffle=True, num_workers=config['workers'], pin_memory=True, drop_last=True
            )
            ...
            dataset_seg_val = SegMultiHeadList(
                config['data_dir'], 'val', val_tf)
            ...
            self.optimizer = torch.optim.SGD([
                {'params': self.model.pretrained.parameters(),
                 'lr': config['lr']},
                {'params': self.model.scratch.parameters(), 'lr': config['lr'] * 0.01}],
                # {'params': sel.model.sigma.parameters(), 'lr': config['lr'] * 0.01}],
                momentum=config['momentum'],
                weight_decay=config['weight_decay'])

            if config['lr_mode'] == 'step':
                def lambda_func(e): return 0.1 ** (e // config['step'])
            elif config['lr_mode'] == 'poly':
                def lambda_func(e): return (1 - e / self.epochs) ** 0.9
            else:
            ...
            test_tf = transforms.Compose([
                transforms.ToTensorMultiHead(),
                transforms.Normalize(mean=config['data_mean'], std=config['data_std'])])

            dataset_at_test = SegMultiHeadList(
                config['data_dir'], 'var_attribute', test_tf, ms=True, scale=self.scales)
            dataset_af_test = SegMultiHeadList(
                config['data_dir'], 'var_affordance', test_tf, ms=True, scale=self.scales)
            dataset_seg_test = SegMultiHeadList(
                config['data_dir'], 'var', test_tf, ms=True, scale=self.scales)

            self.test_loader = DataLoader(
                ConcatSegList(dataset_at_test, dataset_af_test,
                              dataset_seg_test),
        ...
        if self.mode == 'train':
            for epoch in range(self.start_epoch, self.epochs):
                self.writer.add_text(
                    f'Train epoch: {epoch}', f'pretrained_lr: {self.scheduler.get_last_lr()[0]:.06f}')
                self.writer.add_text(
                    f'Train epoch: {epoch}', f'scratch_lr: {self.scheduler.get_last_lr()[1]:.06f}')
            ...
            self.test(save_vis=True, output_dir='output')
        else:
            raise ValueError(f'Unknown exec mode {self.mode}')
    ...
    def train(self, epoch, print_freq):
        ...
        for k, v in self.task_dict.items():
            ...
            losses_array_list.append([AverageMeter() for _ in len(v)])
        ...
        for i, in_tar_name_pair in enumerate(self.train_loader):
            ...
            for index, (input, target, name) in enumerate(in_tar_name_pair):
                ...
                loss_array = [self.criterion(
                    output[idx], target[idx]) for idx in range(len(output))]
                loss = sum(loss_array)
                ...
                scores_list[index].update(
                    np.mean(scores_array), input.shape[0])
            ...
            if index == 2:
                ...
                for index_new, (input_new, target_new, _) in enumerate(in_tar_name_pair):
                    ...
                    target_var_new = [target_new[idx].to(
                        self.device) for idx in range(len(target_new))]
                    output_new, _, _ = self.model(input_new, index_new)
                    loss_array_new = [self.criterion(
                        output_new[idx], target_var_new[idx]) for idx in range(len(output_new))]
                ...
                if i % print_freq == 0:
                    ...
                    for idx, it in enumerate(self.task_list[index]):
                        ...
                        self.writer.add_scalar('train_task_' + it + '_loss_val', losses_array_list[index][idx].val,
                                               global_step=epoch * len(self.train_loader) + i)
                        self.writer.add_scalar('train_task_' + it + '_loss_avg', losses_array_list[index][idx].avg,
                                               global_step=epoch * len(self.train_loader) + i)
                    batch_time = batch_time_list[index]
                    data_time = data_time_list[index]
                    ...
                    self.logger.info(f'File name is: {",".join(name)}')
                    self.writer.add_scalar('train_' + str(index) + '_losses_val', losses_list[index].val,
                                           global_step=epoch * len(self.train_loader) + i)
                    self.writer.add_scalar('train_' + str(index) + '_losses_avg', losses_list[index].avg,
                                           global_step=epoch * len(self.train_loader) + i)
                    self.writer.add_scalar('train_' + str(index) + '_score_val', scores_list[index].val,
                                           global_step=epoch * len(self.train_loader) + i)
                    self.writer.add_scalar('train_' + str(index) + '_score_avg', scores_list[index].avg,
                                           global_step=epoch * len(self.train_loader) + i)
        for i in range(3):
    ...
    def validate(self, epoch, print_freq):
        ...
        for i in range(3):
            losses_list.append(AverageMeter())
            losses_array_list.append([AverageMeter()
                                     for _ in len(self.task_list_array[i])])
            score_list.append(AverageMeter())

        for i, pairs in enumerate(self.val_loader):
            for index, (input, target) in enumerate(pairs):
                input = input.to(self.device)
                target = [target[idx].to(self.device, non_blocking=True)
                          for idx in range(len(target))]
                ...
                if index < 2:
                    scores_array = [mIoU(output[idx], target[idx])[1]
                                    for idx in range(len(output))]
                elif index == 2:
                    scores_array = [mIoU(output[idx], target[idx])
                                    for idx in range(len(output))]
                else:
                    raise ValueError(f'Unkwon task index')
                ...
                if i % print_freq == 0:
                    ...
                    self.writer.add_text(f'Test: [{i}/{len(self.val_loader)}]\t',
                                         f'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                                         f'Score {score.val:.3f} ({score.avg:.3f})')
            score.update(np.nanmean(
                [score_list[0].val, score_list[1].val, score_list[2].val]))
            if i % print_freq == 0:
                self.writer.add_text(f'Test: [{i}/{len(self.val_loader)}]\t',
        ...
        self.writer.add_scalar(
            'val_score_average', score.avg, global_step=epoch)
    ...
    def test(self, save_vis=False, has_gt=False, output_dir=None):
        ...
        for i in range(3):
            ...
            if i < 2:
                ...
                assert 0 == 1
            for _ in range(len(self.task_list_array[i])):
                hist_array_array[i].append(
                    np.zeros((num_classes, num_classes)))
                hist_array_array_acc[i].append(
                    np.zeros((num_classes, num_classes)))

        end = time.time()

        for i, in_tar_pair in self.test_loader:
            for index, input in enumerate(in_tar_pair):
                if index < 2:
                    ...
                    assert 0 == 1
                task_list = self.task_list_array[index]
                ...
                self.logger.info('file name is %s', name)
                ...
                for image in images:
                    ...
                    final, _, _ = self.model(image, index)
                    final_array = list()
                    for entity in final:
                ...
                for label_idx in range(len(outputs[0])):
                    tmp_tensor_list = []
                    for out in outputs:
                        tmp_tensor_list.append(
                            self.resize_4d_tensor(out[label_idx], w, h))
                ...
                if save_vis:
                    for idx in range(len(pred)):
                        ...
                        self.save_output_images(
                            pred[idx], file_name, output_dir)
                        self.save_colorful_images(pred[idx], file_name, output_dir + '_color',
                                                  PALETTE)
                        if index == 2:
                            gt_name = (name[0][:-4] +
                                       task_list[idx] + '_gt.png',)
                            label_mask = (label[idx] == 255)

                            self.save_colorful_images(
                                (label[idx]-label_mask*255).numpy(), gt_name, output_dir + '_color', PALETTE)
                ...
                if has_gt:
                    ...
                    for idx in range(len(label)):
                        ...
                        hist_array_array[index][idx] = np.bincount(num_classes * label[idx].astype(
                            int) + pred[idx], minlength=num_classes**2).reshape(num_classes, num_classes)
                        hist_array_array_acc[index][idx] += hist_array_array[index][idx]

                        map_score_array.append(
                            round(np.nanmean([it * 100.0 for it in eval(iou_compute_cmd)]), 2))

                        self.logger.info('===> task${}$ mAP {mAP:.3f}'.format(
                            task_list[idx],
                ...
                batch_time = batch_time_array[index]
                data_time = data_time_array[index]
                self.logger.info(f'Eval: [{i}/{len(self.test_loader[index])}]\t'
                                 f'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                                 f'Data {data_time.val:.3f} ({data_time.avg:.3f})\t')
        if has_gt:  # val
            ious_array = list()
